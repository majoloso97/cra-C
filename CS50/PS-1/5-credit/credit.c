...
int is_checksum_valid(long number) {
    // Validates if card number has a valid checksum by iterating over each
    // digit on the 10^n position. It simply aggregates the digit if the
    // position is odd. If the position is even, the operation follows the
    // Luhn's algorithm by multiplying the digit by two, and aggregating this
    // product if such product is less than 10. Otherwise, it aggregates the sum
    // of the digits of the product. Finally, if the modulo base 10 of the
    // aggregations of both odd and even positions equals 0, the function
...
int get_first_2_digits_value_from_position(long number, int position) {
    ...
    // E.g. get_first_2_digits_value_from_position(72256, 5) returns 72, which
    // are the first two digits from the 10^5 place of the provided number
